...
class GuardConfig:
    """Bounded correlation settings for the audit-only guard."""

    lookback_seconds: float = 300.0
    cohort_window_seconds: float = 2.0
    ambiguity_margin_seconds: float = 2.0
    max_pre_reference_seconds: float = 2.0
    max_post_reference_seconds: float = 30.0
    future_clock_skew_seconds: float = 5.0

    def validate(self) -> None:
        bounds = {
            "lookback_seconds": (1.0, 3600.0),
            "cohort_window_seconds": (0.1, 10.0),
            "ambiguity_margin_seconds": (0.1, 30.0),
            "max_pre_reference_seconds": (0.0, 5.0),
            "max_post_reference_seconds": (1.0, 60.0),
...
def load_config(root: Path | None = None) -> GuardConfig:
    ...
    deprecated = {"mode", "terminate_timeout_seconds"}
